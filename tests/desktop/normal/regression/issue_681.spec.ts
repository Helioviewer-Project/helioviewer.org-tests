...
test("Issue 681, Duplicate event labels should be handled correctly", async ({ page }, info) => {
  ...
  await page.route("*/**/*action=events&format=simpletree&sources=CCMC*", async (route) => {
...
// Mock data for CCMC events with duplicate labels — simpletree shape:
// { "<source>>><eventtype>": [ <event-instance>, ... ] }
// Each event carries a path "<source>>><eventtype>>><frm>".
const CCMC_DUPLICATE_LABELS_JSON = {
  "CCMC>>DONKI": [],
  "CCMC>>Solar Flare Predictions": [
    {
      path: "CCMC>>Solar Flare Predictions>>AMOS",
      active: "true",
...
// Mock data for HEK events with duplicate labels — simpletree shape.
// Empty entries for the unrelated event types preserve the same key set the
// real API returns; only "HEK>>Active Region" is populated with the test data.
