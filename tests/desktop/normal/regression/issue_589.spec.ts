 ...
 */
test("Issue 589, names with same frm name , creating problems for highlighting features", async ({
  page,
  browser
}, info) => {
  ...
  await page.route("**/*action=events&format=simpletree&sources=HEK*", async (route) => {
    ...
    // Drop CH/AR groups from the real response so we can swap in our test data below.
    delete respJson["HEK>>Coronal Hole"];
    delete respJson["HEK>>Active Region"];

    respJson["HEK>>Coronal Hole"] = [
      {
      ...
      },
      {
        ...
        hpc_boundcc: "POLYGON((354.9 851.7,355.5 852.3,374.1 875.1,465.9 829.5,380.7 849.3,360.9 827.7,354.9 851.7))",
    ...
    // Dump respJson for inspection in the HTML report's Attachments section.
    ...
    await info.attach(`hek-events-mock-${Date.now()}.json`, {
      body: JSON.stringify(respJson, null, 2),
