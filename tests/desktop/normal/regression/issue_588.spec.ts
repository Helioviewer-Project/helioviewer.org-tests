...
test("Issue 588, special characters in event tree causing to tree to fail", async ({ page, browser }, info) => {
  ...
  await page.route("**/*action=events&format=simpletree&sources=HEK*", async (route) => {
    // Fetch original response.
    const response = await route.fetch();
    // Add a prefix to the title.
    let newJson = await response.json();

    // Drop the existing Topological Object group so we can swap in ours below.
    delete newJson["HEK>>Topological Object"];

    // Swap the "Topological Object" with the problem one
    newJson["HEK>>Topological Object"] = [
      {
        path: "HEK>>Topological Object>>SSW PFSS v2 + null point finder v20180808",
        active: "true",
      ...
      },
      {
        ...
        gs_thumburl: "",
        hcr_checked: "false",
        hgc_bbox: "POLYGON((-157.146 38.7029,-153.146 38.7029,-153.146 42.7029,-157.146 42.7029,-157.146 38.7029))",
        hgc_boundcc: "",
    ...
    // Dump newJson for inspection in the HTML report's Attachments section.
    // page.route fires per matching request; the timestamp suffix keeps
    // multiple attachments distinct so refetches don't clobber each other.
    await info.attach(`hek-events-mock-${Date.now()}.json`, {
